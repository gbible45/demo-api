...
public final class CsrfFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		String unitTestMode = System.getProperty("JUNIT_TEST_MODE");
		String referer = request.getHeader("Referer");
		boolean notCsrf = false;
		if (!requireCsrfProtectionMatcher.matches(request)) {
			notCsrf = true;
		} else {
			if (requestUri.indexOf("/api/comm/") == 0) {
				notCsrf = true;
			} else {
				if ("TRUE".equals(unitTestMode)) {
					notCsrf = true;
				} else {
					if (Strings.isNotEmpty(referer) && referer.indexOf("swagger-ui.html") > 0) {
						notCsrf = true;
					}
				}
			}
